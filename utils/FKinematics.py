...
class ForwardKinematics(object):
    ...
    def forward(self, rotation: torch.Tensor, position: torch.Tensor, offset: torch.Tensor, order='xyz', world=True):
        ...
        norm = torch.nn.functional.softplus(norm)
        # norm = torch.zeros_like(norm_original)
        # norm[norm_original < 1e-10] = 1
        ...
        for i, pi in enumerate(self.topology):
            ...
            transform[..., i, :, :] = torch.matmul(transform[..., pi, :, :], transform[..., i, :, :].clone())
