...
def build_t_pose(file: BVH_file, joint, parent_obj, all_obj, n_character):
    if joint != 0:
        offset = mathutils.Vector(file.offsets[joint])
        new_bone = add_bone(offset, parent_obj, file.names[joint] + '%s_bone' %(str(n_character)))
        new_joint = add_joint(parent_obj.location + offset, new_bone, file.names[joint] + '%s_end' %(str(n_character)))
        all_obj.append(new_bone)
        all_obj.append(new_joint)
    else:
        new_joint = add_joint(mathutils.Vector((0., 0., 0.)), None, file.names[joint] + '%s_end' %(str(n_character)))
        all_obj.append(new_joint)

    for i in range(len(file.topology)):
        if file.topology[i] == joint:
            build_t_pose(file, i, new_joint, all_obj, n_character)

def build_t_pose_trunc(file: BVH_file, joint, parent_obj, all_obj, skel_top, n_character):
    if joint != 0:
        if file.names[joint] in skel_top:
            offset = mathutils.Vector(file.offsets[joint])
            new_bone = add_bone(offset, parent_obj, file.names[joint] + '%s_bone' %(str(n_character)))
            new_joint = add_joint(parent_obj.location + offset, new_bone, file.names[joint] + '%s_end' %(str(n_character)))
            all_obj.append(new_bone)
    ...
    for i in range(len(file.topology)):
        if file.topology[i] == joint and file.names[joint] in skel_top:
            build_t_pose_trunc(file, i, new_joint, all_obj, skel_top,n_character)
...
def set_animation_frompos(position, frame_end, frametime,joints):
    bpy.context.scene.frame_start = 0
    ...
    print('Set fps to', bpy.context.scene.render.fps)
    print(frame_end, 'frames in total')

    for frame in range(0, frame_end):
        if frame % 100 == 99:
            print('[{}/{}] done.'.format(frame+1, frame_end))
        for j in range(len(joints)):
            joints[j].location = position[frame,j,:]
            joints[j].keyframe_insert(data_path='location', frame=frame)

    bpy.context.scene.frame_current = 0

def set_animation_fromrot(rotation, position,frame_end, frametime,joints):
    bpy.context.scene.frame_start = 0
    bpy.context.scene.frame_end = frame_end - 1
    ...
    for frame in range(0, frame_end):
        joints[0].location = position[frame,0, :]
        joints[0].keyframe_insert(data_path='location', frame=frame)
        if frame % 100 == 99:
            print('[{}/{}] done.'.format(frame+1, frame_end))
        for j in range(len(joints)):
            joints[j].rotation_mode = 'QUATERNION'
            joints[j].rotation_quaternion = mathutils.Quaternion(rotation[frame, j, :])
            joints[j].keyframe_insert(data_path='rotation_quaternion', frame=frame)
...
def load_file(file_name):
    ...
    return file, all_obj, all_joints

def load_bvh(file_name,n_character):
    print('Loading BVH file......')
    file = BVH_file(file_name)
    print('Loading BVH file done.')
    ...
    print('Building T-Pose......')
    all_obj = []
    build_t_pose(file, 0, None, all_obj,n_character)
    print('Building T-Pose done.')
    ...
    for j in range(file.joint_num):
        name = file.names[j]
        for obj in all_obj:
            if obj.name == name + '%s_end' %(str(n_character)):
                all_joints.append(obj)
                break
...
# def load_bvh(file, n_character = None):
    ...
    # print('Building T-Pose......')
    # all_obj = []
    # build_t_pose_trunc(file, 0, None, all_obj,aj_skeleton,n_character)
    # print('Building T-Pose done.')

    # index = []

    # for joint_name in file.names:
    #     if joint_name in aj_skeleton:
    #         index.append(file.names.index(joint_name))

    # file.names = list(np.array(file.names)[index])
    # print('Loading keyframes......')

    # #pairing object order and file.animation's order
    # all_joints = []
    # for j in range(len(file.names)):
    #     name = file.names[j]
    #     for obj in all_obj:
    #         # if n_character == None:
    #         if obj.name == name + '%s_end' %(str(n_character)):
