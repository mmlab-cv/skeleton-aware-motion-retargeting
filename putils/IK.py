import sys
import torch
from putils.IKinematics import InverseKinematics
from putils.bvh_parser import BVH_file
from tqdm import tqdm
import utils.BVH as BVH
import putils.Animation_deep as Animation
from putils.Quaternions_old import Quaternions


L = 8  # #frame to look forward/backward
...
def fix_foot_contact(input_file, foot_file, output_file, ref_height):
    ...
    anim_gt, _,_ = BVH.load(foot_file)
    ...
    rot_gt = torch.tensor(anim_gt.rotations.qs, dtype=torch.float)
    ...
    anim.rotations[:,0,:] = Quaternions(rot_gt[:,0,:].numpy())
